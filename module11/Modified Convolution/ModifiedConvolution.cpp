...
// truncation occurs here, this is expected and helps the calculation be correct
const unsigned int maskRadius = maskWidth / 2;

const unsigned int outputSignalWidth = inputSignalWidth - (maskRadius * 2);
const unsigned int outputSignalHeight = inputSignalHeight - (maskRadius * 2);
	...
	if (args.debug) {
		...
		for (int r = 0; r < maskHeight; r++) {
			...
			for (int c = 0; c < maskWidth; c++) {
				if (c != 0)
					std::printf("+");
				std::printf("%d ", inputSignal[r][c]);
				rowSum += inputSignal[r][c];
			}
